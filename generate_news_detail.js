...
const mainEndIndex = templateContent.indexOf('</main>') + 7;
const footerContent = templateContent.substring(mainEndIndex);

newsData.forEach(item => {
    // Modify active nav link from tin-tuc.html if needed (tin-tuc.html already has active class on tin-tuc)
...
// Update tin-tuc.html to link to the new pages
let newTinTucContent = templateContent;

// Since the regex might be tricky if excerpt contains special characters, let's do a simpler replacement
newTinTucContent = templateContent;
newsData.forEach(item => {
    newTinTucContent = newTinTucContent.replace(
        `<h2 class="news-title"><a href="#">${item.title}</a></h2>`,
        `<h2 class="news-title"><a href="${item.file}">${item.title}</a></h2>`
    );
});

// Replace all <a href="#" class="news-readmore"> with sequential links
let readMoreIndex = 0;
newTinTucContent = newTinTucContent.replace(/<a href="#" class="news-readmore">Xem thêm<\/a>/g, () => {
    const file = newsData[readMoreIndex] ? newsData[readMoreIndex].file : '#';
    readMoreIndex++;
    return `<a href="${file}" class="news-readmore">Xem thêm</a>`;
});

fs.writeFileSync(templatePath, newTinTucContent, 'utf8');
console.log('Updated tin-tuc.html with proper links');
